...
import com.kenijey.weaponlevels.leveling.EnemyLevel;
import com.kenijey.weaponlevels.network.PacketEnemyLevel;
import com.kenijey.weaponlevels.util.EnumAttributeModifierOperations;
			...
			if (!event.getTarget().world.isRemote)
			{
				...
				if (mob != null)
				{
					...
					if (enemyLevel != null)
					{
						...
						{	
							WeaponLevels.network.sendToAll(new PacketEnemyLevel(enemyLevel.getEnemyLevel(), mob.getEntityId()));
	...
	private void setAttributeModifiers(EntityMob mob, int level)
	{
		if (level == 1)
		{
			if (mob.getEntityAttribute(SharedMonsterAttributes.MAX_HEALTH).getModifier(UUID.fromString("e8da868e-5a50-4376-87b9-35701f9f937a")) == null && mob.getEntityAttribute(SharedMonsterAttributes.ATTACK_DAMAGE).getModifier(UUID.fromString("5d17b4c2-2043-45a3-bbb8-b7da6b054a23")) == null)
			{
				...
				//WeaponLevels.LOGGER.info(mob + " mob health " + mob.getHealth() + " / " + mob.getMaxHealth());
			}
		}
